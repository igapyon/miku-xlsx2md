...
git に入れない実データや一時検証用の `.xlsx` は `workplace/` に配置します。このディレクトリは `.gitignore` 対象です。

`workplace/` は、浅い場所に置きたい手元作業用の一時置き場です。中に置くデータの種類やサブディレクトリ構成は都度変わりえます。

以前の文書では同種の置き場を `local-data/` と表現している箇所がありますが、現在は `workplace/` を優先します。

`workplace/` で使う一部サンプルの取得元メモ:
...
- Markdown 保存と ZIP 保存では `encoding / bom` を切り替えられる
- fixture と `workplace` を使った検証を継続している

- 未対応または将来改善
- レイアウト中心シートでは軽量な section grouping を実装済みだが、専用の意味分解器としての `セクション分割ブロック` はまだ導入していない
...
- Markdown 保存と ZIP 保存では `encoding / bom` を扱う
...
- `encoding / bom`: Markdown 単体保存と ZIP 保存のどちらにも適用する
...
- 現在は軽量実装として、縦方向のギャップとアンカー位置を使ってセクションを分け、Markdown 上では `---` で区切る
...
- 軽量な section grouping はあるが、専用の `セクション分割ブロック` として意味解釈を強める段階にはまだ進んでいない
- フォーム風領域は現時点では保守的に扱う
  - 表として残す場合がある
...
これは厳密な全件一覧ではなく、現行構成の把握に必要な主要部分だけを抜いた概略である。
...
│   ├── rich-text-markdown-rendering.md
│   ├── table-detection-border-priority.md
│   └── xlsx-formula-subset.md
├── scripts/
│   ├── build-miku-xlsx2md.mjs
...
│   ├── xlsx2md-cli.test.js
│   ├── xlsx2md-sheet-markdown.test.js
│   ├── xlsx2md-table-detector.test.js
│   ├── xlsx2md-formula-parser.test.js
│   ├── xlsx2md-text-encoding.test.js
│   └── fixtures/
│       ├── README.md
│       ├── table/
│       ├── display/
