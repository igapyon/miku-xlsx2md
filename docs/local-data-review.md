...
git 管理外の手元実データについて、`xlsx2md` の重点確認対象を整理するメモ。

文書名の `local-data` は過去の呼び方に由来するが、実データの置き場は固定しない。
...
2026-03-18 時点で、対象の Excel 系ファイルは 10 件。
...
- 2026-03-18 時点では、観測対象の実データについて AST parse は全件 `ast_ng 0`
- そのため、次段は parser 未対応の穴埋めではなく、AST evaluator の適用範囲整理と表示差分確認が中心になる
- `dynamic array / spill` については、実データ直確認の範囲では明確な `A1#` 利用例は未確認
...
ここでいう優先順は、git 管理外の手元実データをどの順で確認するかを意味する。`tests/fixtures/` にある共有 fixture の優先度を直接表すものではない。

| 優先 | Workbook / sheet | 何を見るか | 今の主な課題 |
| --- | --- | --- | --- |
| 1 | `TFc2b640... / イベント プランナー` | 多画像・多結合・複数セクションの分解 | 広い merge 領域が巨大表になりやすい |
| 2 | `TF97739... / 月間プランナー` | カレンダー/ボード系のまとまり | 曜日列ごとの小表に割れやすい |
| 3 | `TF0ffdef... / 老後資金プランナー` | 入力パネル + 詳細表 + グラフの分離 | シート全体が巨大表に吸われやすい |

補助確認として、次も引き続き有効。

- `TF2a72be1c...`
  - structured reference / defined name / evaluator 優先順の表示差分確認
- `TFe6ae6c3...`
  - リスト化や会計書式ゼロ値の軽量回帰確認
...
- 現状 Markdown では `C10-AM14` などの広い merge 領域がそのまま巨大表として出ており、`[←M←]` が大量に並ぶ
...
- ただし、2026-03-18 時点の実データ直確認では、明示的な spill 利用例は未確認
