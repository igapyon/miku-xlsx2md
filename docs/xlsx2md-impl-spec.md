...
現行実装では `downloadCurrentMarkdown()` は `createCombinedMarkdownExportPayload(...)` を通じて、保存直前に Markdown 文字列を `Uint8Array` へエンコードしてから Blob 化する。したがってブラウザ UI の Markdown 単体保存でも、`encoding` / `bom` の指定を反映できる。

MIME type も `textEncodingHelper.createTextMimeType(...)` を通じて組み立てられ、少なくとも `utf-8`、`utf-16le`、`utf-16be`、`utf-32le`、`utf-32be` に追随する。`shift_jis` は BOM を許可しない。
...
ZIP 保存も `createWorkbookExportArchive(...)` が `createExportEntries(...)` を経由して Markdown エントリを組み立てるため、ZIP 内 Markdown も同じ `encodeMarkdownText(...)` を通る。つまり Markdown 単体保存と ZIP 保存は、同一のエンコード層を共有している。

したがって現時点の残件は `encoding` / `bom` の未実装ではなく、既定値の妥当性、README / spec / impl-spec 間の説明整合、ブラウザ runtime における `shift_jis` 利用可否の扱いを継続確認することである。
